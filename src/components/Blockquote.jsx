...
function BlockquoteWithImage({ author, children, className, image }) {
  return (
    ...
    >
      <blockquote className="text-abbey-600 col-span-2 text-xl/7 sm:col-span-7 sm:col-start-6 sm:row-start-2">
        {typeof children === 'string' ? <p>{children}</p> : children}
      </blockquote>
      <div className="bg-abbey-100 col-start-1 row-start-2 overflow-hidden rounded-xl sm:col-span-5 sm:row-span-full sm:rounded-3xl">
        <Image
          alt=""
          {...image}
      ...
      <figcaption className="text-abbey-950 text-sm sm:col-span-7 sm:row-start-3 sm:text-base">
  ...
  return (
    <Border position="left" className={clsx('pl-8', className)}>
      <figure className="text-sm">
        <blockquote className="text-abbey-600 [&>*]:relative [&>:first-child]:before:absolute [&>:first-child]:before:right-full [&>:first-child]:before:content-['“'] [&>:last-child]:after:content-['”']">
          {typeof children === 'string' ? <p>{children}</p> : children}
        </blockquote>
        <figcaption className="text-abbey-950 mt-6 font-semibold">
